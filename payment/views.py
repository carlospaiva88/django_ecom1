...
def process_order(request):
    if request.POST:
        ...
        if request.user.is_authenticated:
            ...
            user = request.user
            # Create Order
            create_order = Order(user=user, full_name=full_name, email=email, shipping_adress=shipping_adress, amount_paid=amount_paid)
            create_order.save()
            # Add order items
            # Get the order ID
            ...
            # not logged in
            # Create Order
            create_order = Order(full_name=full_name, email=email, shipping_adress=shipping_adress, amount_paid=amount_paid)
            create_order.save()
            # Add order items
    ...
    if request.POST:
        ...
        paypal_dict = {
            ...
            'no_shipping': '2',
            'invoice': my_Invoice,
            'currency_code': 'USD', # BRL, EUR
            'notify_url': f'{scheme}://{host}{reverse("paypal-ipn")}',
            'return_url': f'{scheme}://{host}{reverse("payment_success")}',
            'cancel_return': f'{scheme}://{host}{reverse("payment_failed")}',
        }
